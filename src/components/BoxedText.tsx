...
const BoxedText = ({as: Tag = 'h2', imgRef, content, className, backgroundColor, blendMode, direction = 'right'}: BoxedTextProps) => {
    ...
    const [boxMargin, textPadding, boxPlacement] = useMemo(() => {
        const bx = {left: 'ml-2 md:ml-4', right: 'mr-2 md:mr-4', small: 'ml-2 md:mr-4'}[direction]
        const tp = {left: 'pl-2 md:pl-4', right: 'pr-2 md:pr-4', small: 'pl-2 md:pr-4'}[direction]
        const bp = {
    ...
    return <div className={className}>
        <div className={`relative inline-block ${boxMargin}`}>
            ...
            <div className={`absolute inset-0 bg-clip-text ${textPadding} pb-4 z-5 ${backgroundColor} boxed-text-stroke text-transparent`}>{content}</div>
            {/* stroke image background */}
            {imgRef
                ? <div ref={ref} className={`absolute inset-0 bg-clip-text ${textPadding} pb-9 z-10 ${blendMode} boxed-text-stroke text-transparent`} style={{backgroundImage: `url("${imgRef.current?.src}")`, backgroundSize: imgSize, backgroundPosition: imgPos}}>{content}</div>
                : <></>
            }
        </div>
    </div>
